...
import { Eraser, Palette, Pencil, Trash2, Smile, ImagePlus, Video, X, Sparkles } from "lucide-react";
...
const FILTERS = [
  ...
  // --- Effets exclusifs : combinaisons de filtres ffmpeg (rgbashift,
  // edgedetect, pixelisation par ré-échantillonnage "neighbor", courbes
  // par canal...) qu'on ne trouve pas dans l'appareil photo d'un téléphone
  // ni dans les filtres standards des autres apps. Rendu réel généré
  // côté serveur ; l'aperçu CSS ci-dessous n'est qu'une approximation.
...
function DrawingCanvas({ platform, dims }) {
  ...
  // Le dessin et la gomme partagent le même tracé libre ; seule change la
  // méthode de composition ("source-over" pour peindre, "destination-out"
  // pour effacer les pixels déjà posés — trait, émoji ou photo tamponnée),
  // ce qui permet de gommer précisément une zone sans tout effacer.
  ...
  const endDraw = () => {
    ...
    const ctx = canvasRef.current.getContext("2d");
    ctx.globalCompositeOperation = "source-over"; // remise à zéro pour les tampons émoji/photo
    exportOverlay();
  };
    ...
    <div>
      <canvas
        ...
        className={`absolute inset-0 w-full h-full touch-none ${
          mode === "pen" ? "cursor-crosshair" : mode === "eraser" ? "cursor-cell" : "cursor-copy"
        }`}
      ...
      <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between gap-2 bg-black/50 backdrop-blur rounded-lg px-2 py-1.5">
        ...
        <button
          type="button"
          onClick={handleClear}
          title="Effacer tout le calque (dessin, émojis, photo)"
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4">
      ...
      <p className="text-xs text-gray-400 mb-3">
        Dessine, gomme précisément une zone, ajoute des émojis ou une photo, incruste une vidéo
        (PiP) et anime des stickers (fleur, papillon), indépendamment pour chaque plateforme.
        Inclut aussi des effets <Sparkles size={11} className="inline -mt-0.5 text-purple-500" /> exclusifs
        (glitch RGB, VHS, néon, pixel art, infrarouge...) introuvables dans l'appareil photo d'un
        téléphone ou les filtres classiques des autres applis.
      </p>
